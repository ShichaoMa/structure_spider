...
2 若执行分类抓取则需要配置item_xpath = (), page_xpath = ()，其中：
    1）item_xpath元组中的元素为分类页中每个item链接所对应的xpath表达式
    2）page_xpath元组中的元素可以是下一页链接所所对应的正则表达式，或者其它规则，详见https://github.com/ShichaoMa/webWalker/wiki/%E9%80%9A%E7%94%A8%E6%8A%93%E5%8F%96%E9%85%8D%E7%BD%AE%E6%96%B9%E6%B3%95
...
from .utils import CustomLoader, enrich_wrapper, refresh, urldecode, re_search, safely_json_loads
...
class BaiduMp3Spider(StructureSpider):
    ...
    def enrich_data(self, item_loader, response):
        ...
        node_list.append(("lyrics", item_loader,
                          {"url": "".join(response.xpath('//div[@id="lyricCont"]/@data-lrclink').extract())}))
        node_list.append(("source_url", item_loader,
                          {"url": "http://tingapi.ting.baidu.com/v1/restserver/ting?method=baidu.ting.song.play&"
                                  "format=jsonp&callback=jQuery_%s&songid=%s&_=%s" % (
                                      int(time.time() * 1000), re_search(r"song/(\d+)", response.url),
                                      int(time.time() * 1000))}))
